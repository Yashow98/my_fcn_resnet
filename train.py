...
def create_model(aux, num_classes, pretrain=True):
    ...
    if pretrain:
        weights_dict = torch.load("./fcn_resnet50_coco.pth", map_location='cpu') # 载入预训练权重到CPU中
...
def main(args):
    ...
    if args.aux: # 如果启用辅助分类器的话，加载相应的权重
...
def parse_args():
    ...
    parser.add_argument("-b", "--batch-size", default=1, type=int) # 根据gpu显存进行设置
